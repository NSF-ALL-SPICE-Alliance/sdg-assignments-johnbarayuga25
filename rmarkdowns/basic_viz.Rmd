...
```{r}
ggplot(data = sdr_data, aes(x= goal_6_score, y = goal_9_score, color = goal_2_score)) +
  geom_point()
```
Interpretation: Countries with lower SDG 2 scores tend to have lower SDG 6 and 9 scores as well.  Countries with higher SDG 2 scores, generally above a value of 40, seem to correlate positively with higher values of SDG 6 and 9 scores. 
...
ggplot(sdr_data, aes(x = goal_13_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black")
```
Interpretation: Countries that fall in the higher end of the SDG 13 spectrum tend to belong to the Sub-Saharan Africa regions, whereas those that are on the lower end tend to belong to the OECD and MENA regions. 
